...
class WiredSpec extends FlatSpec {
  ...
  it should "generate implicit Provider def for a polymorphic class with a companion" in {
    ...
    // when
    @Wired
    class PolyCompanion[T]
    object PolyCompanion

    object PolyCompanion2
    @Wired
    class PolyCompanion2[T]
  ...
  it should "generate implicit Provider def for a class with a parameters" in {
    // given
    implicit val stringProvider: Provider[String] = Provider.instance("test")
    implicit val intProvider:    Provider[Int] = Provider.instance(10)
    implicit val doubleProvider: Provider[Double] = Provider.instance(20.4)

    // when
    @Wired
    class ComplexCase[T](t: T, name: String, size: Int)
